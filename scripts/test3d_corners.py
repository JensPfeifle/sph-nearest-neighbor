...
def make_fill(fill: float):
    ...
    # fill another eighth across the x axis with zeros
    zeros = np.zeros(field.shape)
    field = np.concatenate([field, zeros], axis=0)
    # fill a quarter across the y axis with zeros
    zeros = np.zeros(field.shape)
    field = np.concatenate([field, zeros], axis=1)
    # then mirror the half of the field we currently have over each axis
    # and set it to be the top half of the field
    field = np.concatenate([field, field[::-1, ::-1, ::-1]], axis=2)
